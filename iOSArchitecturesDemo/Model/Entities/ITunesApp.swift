...
public struct ITunesApp: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.currentVersionReleaseDateString = try container.decode(String.self, forKey: .currentVersionReleaseDateString)
        let dateFormatter = DateFormatter()
        dateFormatter.locale = Locale(identifier: "en_US_POSIX")
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ssZ"
        self.currentVersionReleaseDate = dateFormatter.date(from: self.currentVersionReleaseDateString ?? "")!
    }
